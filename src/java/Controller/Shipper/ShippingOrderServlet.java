...
public class ShippingOrderServlet extends HttpServlet {
    ...
    private ArrayList<Order> searchOrder(HttpServletRequest request, User customer) {
        ...
        try {
            ...
            orderList = orderDAO.searchShippingOrderById(orderId, customer.getId());

        } catch (NumberFormatException e) {
            // If searchParam is not a number, search by user.fullname
            orderList = orderDAO.searchShippingOrderByName(search, customer.getId());
        }
        return orderList;
    }
    ...
    private ArrayList<Order> filterOrder(HttpServletRequest request, User customer) {
        ...
        OrderDAO orderDAO = new OrderDAO();
        ArrayList<Order> orderList = orderDAO.filterShippingOrderList(fromDate, toDate, statusIDStr, customer.getId());
        return orderList;
    }
        ...
        if (orderList.isEmpty()) {
            ...
            for (Order order : orderList) {
                ...
                htmlResponse.append("                <tr>\n")
                        ...
                        .append("                        <a href=\"ShippingOrderDetailServlet?id=").append(order.getId()).append("\" style=\"text-decoration: none; color: black;\">")
                ...
                switch (order.getStatus()) {
                    case "Confirmed":
                        statusButton = "<button style=\"height: 30px\" class=\"btn btn-info\">Đã xác nhận</button>";
                        break;
                    case "Delivering":
                        statusButton = "<button class=\"btn btn-light\" style=\"height: 30px; background-color: aquamarine\" >Đang giao</button>";
                        break;
                    case "Delivered":
                        statusButton = "<button class=\\\"btn btn-light\\\" style=\\\"height: 30px; background-color: purple\\\" >Giao hàng thành công</button>";
                        break;
                    case "Deliveryfailed":
                        statusButton = " <a href=\"#\" class=\"btn btn-light\" style=\"height: 30px; background-color: brown; color: white\">Giao hàng thất bại</a>\n";
                        break;
                }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        OrderDAO orderDAO = new OrderDAO();
        ArrayList<Order> orderList = orderDAO.OrderListOfShipper(customer.getId());

        session.setAttribute("orderList", orderList);

        String htmlResponse = listOrder(request, orderList);
        request.setAttribute("htmlResponse", htmlResponse);
        request.getRequestDispatcher("Views/ShippingOrderList.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("pagination".equals(action)) {
            ...
            orderList = orderDAO.OrderListOfShipper(customer.getId());
